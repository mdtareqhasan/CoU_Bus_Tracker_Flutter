...
class IdCardValidationService {
  ...
  static Future<IdCardValidationResult> validateIdCard(File imageFile, {String role = 'student'}) async {
    try {
      ...
      // If no text detected, still accept (image might be blurry or Bangla only)
      ...
      // If document type detected, extract fields
      if (docType != null) {
      ...
      // No document type detected but text exists - still accept
      return IdCardValidationResult(
        isValid: true,
        documentType: 'unknown',
        rawText: text,
      ...
      // On error, still accept the image
      return IdCardValidationResult(
        isValid: true,
        documentType: 'unknown',
        rawText: '',
  ...
  static String? _extractDesignation(String text) {
    ...
    if (banglaMatch != null) {
      ...
      for (final known in designations) {
        if (desig.toUpperCase().contains(known.toUpperCase())) return known;
      }
      return desig;
    }
